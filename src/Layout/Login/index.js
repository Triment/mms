...
export default function(){
    ...
    function focusFunction(){
        ...
        parentNode.classList.add(style['focus']);
    }
    function blurFunction(){
        let parentNode = this.parentNode.parentNode;
        if(this.value == ''){
            parentNode.classList.remove(style['focus']);
        }
    }
    ...
    const submitHandle = e => {
        ...
        //setUserState({username: 'splub', role: 1, isAuthenticated: true})
    }
    return <div className={style.container}>
            <div className={style.loginBox}>
                <form action="" className={style.form}>
                    <svg id="e59edb86-a3bc-4694-8aac-31e565ca5cfc" xmlns="http://www.w3.org/2000/svg" width="100"
                    height="100" viewBox="0 0 676 676">
                    ...
                    <h2 className={style.title}>欢迎</h2>
                    <div className={style.inputGroup}>
                        ...
                        <div>
                            <h5>用户名</h5>
                            <input type="text" ref={textRef} className={style.input}/>
                        </div>
                    </div>
                    <div className={style.inputGroup}>
                        <div className={style.icon}>
                        ...
                        <div>
                            <h5>密码</h5>
                            <input type="password"  ref={passwordRef} className={style.input}/>
                        </div>
                    </div>
                    <a href="#">忘记密码?</a>
                    <input onClick={submitHandle} type="submit" className={style.btn} value="登录"/>
                </form>
            </div>
        </div>
}